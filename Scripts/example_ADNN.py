...
def plot_and_loss(eval_model, data_source, epoch):
    ...
    with torch.no_grad():
        for i in range(0, len(data_source) - 1, batch_size):
            ...
            output = output.transpose(0, 1)
            target = target.transpose(0, 1)
            total_result = torch.cat((total_result, output[-output_window:].squeeze(2).view(-1).cpu() * max1), 0)
            truth = torch.cat((truth, target[-output_window:].squeeze(2).view(-1).cpu() * max1), 0)
            loss = criterion(output[-output_window:], target[-output_window:])
            total_loss += loss.item()
...
for cell in cells:
    ...
    training_data, val_data, max1 = get_data(size)
    pred = []
    tru_value = []
